...
This OpenSpec-enabled fork is installable through [skills.sh](https://skills.sh/yangpf5271/mattpocock-skills-openspec) (`npx skills@latest add yangpf5271/mattpocock-skills-openspec`), which copies editable skill files into a user's project across Claude Code, Codex, and other Agent-Skills-standard harnesses. The upstream project also ships a native Claude Code plugin; this fork keeps compatible plugin metadata for direct testing while documenting skills.sh as its cross-harness distribution route.
...
- **Codex** — `.codex-plugin/plugin.json` accepts `skills` only as a **single path string** (arrays are rejected with `missing or invalid plugin.json`), and Codex discovers `SKILL.md` files recursively under it. There is no way to name two bucket folders, or to curate a subset, from one path. Two escape hatches were tested and rejected:
  - Pointing at `./skills/` would also ship `deprecated/`, `in-progress/`, and `misc/` — retired, beta, and rarely used skills we deliberately don't promote.
...
The upstream `mattpocock-skills` package was accepted into **Claude Code's official marketplace** — configured name `claude-plugins-official`, source repo `anthropics/claude-plugins-official` — which every Claude Code install has by default. `claude plugins install mattpocock-skills` installs that upstream package, not this OpenSpec fork. This fork's current install wording lives in [.agents/install-block.md](../install-block.md).

The official listing points at the upstream repo's git URL and reads its `.claude-plugin/plugin.json` directly, so it does not depend on this fork's `.claude-plugin/marketplace.json`. This fork retains its marketplace file only as a fallback for direct testing.
