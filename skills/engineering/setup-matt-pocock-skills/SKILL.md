...
description: "首次使用前配置仓库：Configure this repo for the engineering skills: set up its issue tracker, triage label vocabulary, domain doc layout, and optional standard OpenSpec instance. Run once before first use of the other engineering skills."
...
- **Other** (Jira, Linear, etc.): ask the user to describe the workflow in one paragraph, including how to read, create, update, close, reopen, change status/labels, and read/add/remove dependency edges. The skill will record it as freeform prose. If any of those operations are unavailable, record that explicitly so downstream skills stop and ask instead of guessing.
...
3. **Standard config present?** If `openspec/config.yaml` is still missing after init, write the minimal standard config:
   ...
   schema: spec-driven
   ```
   Do not add `context:` or `rules:`. Matt-specific behavior belongs in these skills, not in the shared OpenSpec configuration. If a config already exists with another schema, do not overwrite it; explain that `/to-proposal` targets the `spec-driven` schema and ask the user how to proceed.
