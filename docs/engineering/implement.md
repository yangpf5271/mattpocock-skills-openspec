...
When an OpenSpec change exists, `openspec/changes/<name>/tasks.md` is also an input and the archive record. With promoted tickets, `implement` drives the ticket first and then checks the matching `tasks.md` lines. Without tickets, it works the checklist directly. The OpenSpec instance location comes from `docs/agents/openspec-instance.md`.
...
A normal tracker-only run ends at the commit and does not close the ticket or tick criteria on the tracker. OpenSpec is the exception: when a matching change exists, the skill explicitly keeps `tasks.md` accurate because `openspec archive` reads that checklist for completion. With promoted tickets, tracker state is the collaboration surface and is updated first; the matching OpenSpec checkbox follows. Without tickets, the checklist is worked directly. In either mode, closing the external ticket remains a separate tracker action unless the invocation asks for it.
...
- When an OpenSpec change is active, completed work is reflected in `tasks.md`, with tracker state updated first when promoted tickets exist.
...
Its neighbours are [to-tickets](https://aihero.dev/skills-to-tickets), which produces the tickets it consumes and declares the blocking edges that decide their order; [tdd](https://aihero.dev/skills-tdd), which it drives internally at each seam; and [code-review](https://aihero.dev/skills-code-review), which it runs before committing. In the optional OpenSpec lifecycle it can instead consume `to-proposal`'s checklist directly, or keep that checklist synchronized with promoted tickets, before [archive-proposal](https://aihero.dev/skills-archive-proposal) closes the change. It sits downstream of the planning skills and trusts them.
