...
- **tasks.md** — the OpenSpec implementation checklist: vertical tracer-bullet slices grouped under `## N.` headings, with `- [ ]` checkboxes so progress is machine-trackable. It is not a tracker ticket, but [to-tickets](https://aihero.dev/skills-to-tickets) can later promote each group into a ticket by adding tracker-specific fields and writing a backlink.

It follows the OpenSpec dependency chain in order — proposal unlocks design and specs; those unlock tasks — and stops when the change is ready to implement. The `tasks.md` checklist is written as **vertical slices** so it can either be implemented directly or promoted into tracker tickets without re-breaking the work from scratch.
...
- Its `tasks.md` is understood as the OpenSpec checklist — direct input for lightweight implementation, and the source groups that `to-tickets` can promote into tracker tickets.
