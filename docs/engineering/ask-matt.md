...
The tracker-dependent routes (triage, `to-spec`, `to-tickets`, `implement`) assume [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) has already configured an issue tracker in the repo. The optional OpenSpec route also assumes setup initialized an OpenSpec instance and recorded it in `docs/agents/openspec-instance.md`. The router will happily recommend either route before its prerequisite has been met.
...
- **The main flow**, idea to ship. Grill, spec, tickets, implement, review, with two branches inside it: a prototype detour when a question needs runnable code to settle, and the spec-and-tickets split, which only earns its cost when the build spans more than one session. Projects that keep evolving specs in the repo can layer on the OpenSpec loop: `to-proposal`, optional `to-tickets`, `implement`, then `archive-proposal`. Ticket promotion preserves each group's completion state and adds tracker detail without replacing `tasks.md`. When every OpenSpec task is already complete and no group was ever promoted, ticketing has no collaboration work left to represent, so the route goes straight to `archive-proposal`.
...
- When the project uses OpenSpec, it distinguishes lightweight checklist implementation from the optional ticket-promotion path and ends both at `archive-proposal`.
