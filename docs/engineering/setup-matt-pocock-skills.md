...
```bash
npx skills add yangpf5271/mattpocock-skills-openspec --skill=setup-matt-pocock-skills
```
...
[Source](https://github.com/yangpf5271/mattpocock-skills-openspec/tree/main/skills/engineering/setup-matt-pocock-skills)

## What it does

`setup-matt-pocock-skills` teaches one repo how the engineering skills should behave in it — where issues live, what the triage labels are called, where the domain docs sit, and where its optional OpenSpec instance lives — and records those answers as **config** the other skills read.

It writes config, it does not hard-code behaviour. The engineering chain assumes shared files under `docs/agents/` exist; this skill is the one-time bootstrap that produces them, discovered from your actual repo (`git remote`, existing labels, existing `CONTEXT.md`) and confirmed with you rather than guessed. It is prompt-driven — explore, present what it found, confirm, then write — not a deterministic scaffold.
...
- **OpenSpec** — checked only when `to-proposal` or `archive-proposal` is installed. It initializes the standard `spec-driven` instance with `--tools none` and writes only `schema: spec-driven` when the project has no config. Existing team config is never overwritten; Matt-specific rules stay in the skills.

The output is a set of files under `docs/agents/` — `issue-tracker.md`, `domain.md`, `triage-labels.md` when `triage` is installed, and `openspec-instance.md` when the OpenSpec flow is installed — plus an `## Agent skills` block pointing to them in whichever of `CLAUDE.md` / `AGENTS.md` the repo already uses. Those files are the shared substrate the rest of the toolkit stands on.
...
- When the OpenSpec flow is installed, `openspec/config.yaml` contains at least `schema: spec-driven`, the instance directories exist, and existing team config was not overwritten.
