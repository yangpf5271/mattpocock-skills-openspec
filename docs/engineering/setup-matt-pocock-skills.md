...
`setup-matt-pocock-skills` answers the repository-level questions the installed engineering skills need: where issues live, what the triage labels are called, where the domain docs sit, and, when the OpenSpec lifecycle skills are installed, where the OpenSpec instance lives and whether it is usable. It records the answers as markdown files under `docs/agents/`.
...
| `openspec-instance.md` | `docs/agents/`, only when `to-proposal` or `archive-proposal` is installed |
...
It leads each section with the recommended answer, and skips whatever exploration already settled. Most runs are two confirmations and done; the OpenSpec section appears only when those lifecycle skills are installed.
...
It configures tracker, labels, domain-doc layout, and, only when those skills are installed, the OpenSpec instance. It does not configure interview cadence, question format, tone, or arbitrary per-user preferences. The standing answer is that skills stay opinionated: *"Config is death."* Preferences belong in your `CLAUDE.md` as plain instructions, which every skill already reads.
...
- `docs/agents/issue-tracker.md` and `docs/agents/domain.md` exist, plus `triage-labels.md` if `triage` is installed and `openspec-instance.md` if the OpenSpec lifecycle is installed.
...
- Afterwards, `/to-tickets` publishes without asking you where issues live, `/triage` applies labels rather than inventing them, and `/to-proposal` can find an initialized OpenSpec instance when that flow is installed.
