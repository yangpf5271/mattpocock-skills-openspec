...
| An OpenSpec change already has `tasks.md` groups | `/to-tickets` promotes those groups and writes ticket backlinks; it does not re-break them by default |
...
Upstream is [to-spec](https://aihero.dev/skills-to-spec), which can hand it a settled tracker spec — keep both in one unbroken context window. In the optional OpenSpec flow, [to-proposal](https://aihero.dev/skills-to-proposal) supplies the `tasks.md` groups that this skill promotes without replacing the checklist as the archive record. Downstream is [implement](https://aihero.dev/skills-implement), which builds one ticket per fresh session, drives [tdd](https://aihero.dev/skills-tdd), and keeps the matching OpenSpec checkboxes accurate when present. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
