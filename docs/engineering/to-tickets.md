...
It only changes `tasks.md` boundaries or checkbox items when the checklist is not actually ticket-sized, when it is missing required work that belongs in the OpenSpec archive record, or when you approve a split/merge during the quiz. That is a revision to the OpenSpec implementation checklist, not ordinary ticket backlinking. After publishing, updating, or reusing a ticket, it writes a plain non-checkbox backlink such as `**Ticket:** <url-or-path>` under the matching group. That backlink prevents duplicate tickets on reruns and helps [implement](https://aihero.dev/skills-implement) map tracker work back to the OpenSpec checklist without turning `tasks.md` into the issue tracker.
