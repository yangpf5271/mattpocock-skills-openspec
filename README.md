...
This installs Matt Pocock's official marketplace version, which is available without adding a marketplace first and updates through Claude Code's official plugin flow. Use the `npx skills` command above when you need `/to-proposal`, `/archive-proposal`, or the modified Matt + OpenSpec workflow.
...
It writes the skills into your repo as ordinary files you own and can edit. Nothing updates behind your back; pull the latest changes when you want them with `npx skills@latest update`.
...
- **[archive-proposal](./skills/engineering/archive-proposal/SKILL.md)**: Finalize a finished OpenSpec change: sync its delta specs into the project's main specs, then file the change under the archive.
...
- **[codebase-atlas](./skills/engineering/codebase-atlas/SKILL.md)**: Build and maintain a persistent codebase atlas as committed Markdown plus preview-verified diagram sources and PNG exports: symbol cards, named execution flows, impact rings with risk levels, and per-region freshness, completed targetedly around whatever you are about to change.
